...
const el = (name: string, type: TypeRef, min = 1, max: number | 'unbounded' = 1): IrParticle => ({
  kind: 'element',
  name,
  ns: 'wml',
  type,
...
function schema(parts: {
  ...
  const byKey = <T extends { name: QName }>(xs: readonly T[] = []) =>
      ...
      simpleTypes: [
        {
          ...
          base: str,
          values: [
            { value: 'red', source: SRC },
            { value: 'blue', source: SRC },
          ],
        ...
        },
        {
          kind: 'restriction',
          name: q('ST_Short'),
          base: { kind: 'builtin', name: 'xsd:int' },
          facets: { minInclusive: '1', maxInclusive: '3' },
          dialects: ['transitional'],
          source: SRC,
        },
      ],
