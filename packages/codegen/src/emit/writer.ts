...
export function emitWriter(set: ModelSet, ns: LogicalNs): EmittedModule {
  const file = new SourceFile(generatedBanner(`writers for the ${ns} namespace of the ECMA-376 5th edition schema set`));
...
class WriterContext {
  constructor(readonly set: ModelSet, readonly ns: LogicalNs, readonly file: SourceFile) {}
  rt(name: string, typeOnly = false): string { return this.file.importName(RUNTIME, name, typeOnly); }
  typeName(ref: TypeRef): string {
    if (ref.kind === 'builtin') return builtinTs(ref.name);
    const d = this.set.complexTypes.get(`${ref.ref.ns}#${ref.ref.name}`) ?? this.set.simpleTypes.get(`${ref.ref.ns}#${ref.ref.name}`);
    if (!d) throw new Error(`Unresolved type reference ${ref.ref.ns}#${ref.ref.name}`);
    return ref.ref.ns === this.ns ? d.tsName : this.file.importName(`../${ref.ref.ns}/types.js`, d.tsName, true);
  }
  /**
...
function emitComplexWriter(ct: ModelComplexType, ctx: WriterContext, file: SourceFile): void {
  ...
  file.block(`export function write${ct.tsName}(s: ${sink}, value: ${type}, ctx: ${writeCtx}, localName: string): void {`, () => {
    ...
    if (ct.content.kind === 'simpleContent') {
      ...
      const hasUnknown = slots.length > 0 && !slots.some((x) => x.kind === 'choice' && x.cardinality.repeated);
      for (const slot of slots) emitSlot(ct, slot, ctx, file);
      if (hasUnknown) {
        file.line(`for (const u of value.$unknown ?? []) s.raw(u.node);`);
      }
    }
    file.line('for (const a of value.$unknownAttrs ?? []) s.attr(a.uri || null, a.localName, a.value);');
...
function emitAttribute(a: ModelAttribute, ctx: WriterContext, file: SourceFile): void {
  ...
  file.line(`if (value[${stringLiteral(a.prop)}] !== undefined) s.attr(${ns}, ${stringLiteral(a.name)}, String(value[${stringLiteral(a.prop)}]));`);
}
...
function emitSlot(ct: ModelComplexType, slot: Slot, ctx: WriterContext, file: SourceFile): void {
  ...
  if (slot.kind === 'element') {
    if (!ctx.complex(slot.type)) {
      ...
      const values = slot.cardinality.repeated ? `value[${stringLiteral(slot.prop)}]` : `[value[${stringLiteral(slot.prop)}]]`;
      file.line(`for (const ${one} of ${values}) if (${one} !== undefined) s.text(String(${one}));`);
      return;
    }
    const w = ctx.writerFor(slot.type);
    const one = `v_${safe(slot.prop)}`;
    if (slot.cardinality.repeated) {
      file.block(`for (const ${one} of ${p}) {`, () => file.line(`${w}(s, ${one}, ctx, ${stringLiteral(slot.element.name)});`));
    } else {
  ...
  if (slot.kind === 'choice') {
    ...
    const write = (expr: string) => {
      const branches = slot.alternatives.map((a) => {
        if (!ctx.complex(a.type)) return `if (${expr}.kind === ${stringLiteral(a.tag)}) s.text(String(${expr}.value));`;
    ...
    if (slot.cardinality.repeated) file.block(`for (const ${one} of ${p}) {`, () => write(one).forEach((x) => file.line(x)));
...
function byName(a: { tsName: string }, b: { tsName: string }): number { return a.tsName.localeCompare(b.tsName); }
function builtinTs(name: string): string {
  return name === 'xsd:boolean' ? 'boolean' : name === 'xsd:string' || name === 'xsd:token' ? 'string' : 'number';
}
