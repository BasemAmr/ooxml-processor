...
the ones that are expensive to get wrong and cheap to get right *on paper*. Writing them down first,
...
The hardest-first ordering is for *reading and designing*, not for *doing*.
...
| 05 ⇄ 06 | `P5-01` carries per-cluster source offsets *because* `P6-01` needs them; `P6-13` reflows from the line box `P5-16` invalidates. |
...
Two families are *deliberately* undefined at top level and will not appear in that output, because
